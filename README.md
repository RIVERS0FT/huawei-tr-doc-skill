...
- 支持生成 TR1-TR6 阶段设计文档包
- 默认输出 Markdown 文本
- 支持 Mermaid 用例图和时序图
- 支持软件、AI Agent、仿真系统、平台型项目
- 自动补齐项目背景、项目目标、用例分析、功能分析、核心方案、风险清单、设计检查表
- 对缺失信息自动标记为“设计假设”“约束说明”或“风险”
...
| TR-ALL | TR1-TR6 阶段设计文档包 | 一次性组织完整 TR 阶段文档包，并保持各阶段边界清晰 |
...
`templates/tr-all.md` 用于生成 TR1-TR6 阶段设计文档包。

使用示例：

```text
生成 TR1-TR6 设计文档包：开发 AgentNetworkSimulation，用于模拟多个 agent 之间的消息协作、日志追踪和流程回放。
```

```text
生成完整 TR 文档包：开发一个 AI 自动生成设计文档的 skill。
```
...
- TR2 聚焦需求分解、规格和验收。
- TR3 聚焦总体方案、架构和技术路线。
- TR4 聚焦详细设计、接口、数据结构、异常处理和技术选型。
- TR5 聚焦验证设计、测试场景和质量门禁。
- TR6 聚焦发布、交付、运维和回滚。
...
```text
生成 TR1-TR6 文档包：开发 AgentNetworkSimulation，用于模拟多 Agent 网络通信、日志追踪和拓扑回放。
```
